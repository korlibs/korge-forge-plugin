...
fun Type.isKorimBitmapOrDrawable() = this.instanceOf<Bitmap>() || this.instanceOf<BmpSlice>() || this.instanceOf<Drawable>() || this.instanceOf<Image>()
...
fun ObjectReference.readKorimBitmap32(hintWidth: Int, hintHeight: Int, thread: ThreadReference?): Bitmap32 {
	return when {
		...
		instanceOf<Image>() -> readKorgeImageInternal(thread)
		instanceOf<Drawable>() -> {
...
fun ObjectReference.readKorgeImageInternal(thread: ThreadReference?): Bitmap32 {
	val value = this
	if (!value.instanceOf<Image>()) error("Not a com.soywiz.korge.view.Image")
...
fun ObjectReference.readKorimBitmap32Internal(thread: ThreadReference?): Bitmap32 {
	...
	return Bitmap32(width, height, dataInstance.copyOf(), premultiplied)
}
